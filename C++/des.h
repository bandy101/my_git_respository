...
void DESEncrypt(const char* inBuff, int inSize, const char* outBuff, const char key[8]) {
	int time = inSize / 8;
	int out[64] = { 0 };
	for (int i = 0; i < time; i++) {
		DES_Efun((char*)&inBuff[i * 8], (char*)key, out);
		BitToChar(out, (char*)&outBuff[i * 8], 8);
	}
	if (inSize % 8 != 0) {
		int size = inSize % 8;
		char buff[8] = { 0 };
		memcpy(buff, &inBuff[time * 8], size);
		DES_Efun((char*)&inBuff[time * 8], (char*)key, out);
		BitToChar(out, (char*)&outBuff[time * 8], 8);
	}
}

void DESDecrypt(const char* inBuff, int inSize, const char* outBuff, const char key[8]) {
	int time = inSize / 8;
	int in[64] = { 0 };
	for (int i = 0; i < time; i++) {
		CharToBit((char*)&inBuff[i * 8], in, 8);
		DES_Dfun(in, (char*)key, (char*)&outBuff[i * 8]);
	}
}