        ...
        # for interval_small in xrange(-0,width_boundingbox/2):
            for i in range(-8,int(width_boundingbox/1)-8):
                for refine in range(refine_s,int(width_boundingbox/2)+3):
                    ...
                    p2 = int(p1 + width_boundingbox +refine) #
                    p3 = int(p2 + width_boundingbox + interval_big+i+1)
                    p4 = int(p3 + width_boundingbox +refine)
                    p5 = int(p4 + width_boundingbox +refine)
                    p6 = int(p5 + width_boundingbox +refine)
                    p7 = int(p6 + width_boundingbox +refine)
                    if p7>=length:
                        continue
...
def refineCrop(sections,width=16):
    ...
    for section in sections:
        ...
        new_box[0],new_box[1],new_box[2],new_box[3] = map(int,[new_box[0],new_box[1],new_box[2],new_box[3]])
