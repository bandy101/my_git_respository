 ...
 * @author 廖志群
 * @version 1.00
 * @date 五月  22 2017,17:12
 * 文件上传 控制器 主要是图片服务
 */
@RestController
@RequestMapping(value = "/api/{version}/picture")
public class UploadFileController {
    ...
    @ApiVersion(1)
    public ResultMsg queryFileData(
        ...
        if (!file.isEmpty()) {
            ...
            String pathDir = request.getSession().getServletContext()
                    .getRealPath("/upload/img/" + curTime);// 存放文件夹
    ...
    @ApiVersion(1)
    public ResultMsg queryFileDatas(
    ...
    @ApiVersion(1)
    public @ResponseBody
    byte[] getFile(@PathVariable("fileName") String fileName,@PathVariable("prefix") String prefix,
                   HttpServletRequest request) {
        ...
        try {
            ...
            String path = request.getSession().getServletContext()
                    .getRealPath("/upload/img/"+dateStr+"/" + fileName +"."+ prefix);// 存放位置
