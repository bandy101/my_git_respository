...
class cSysInfo:
    ...
    def loaddata(self,sKey=''):         
        sql="""select 
                    ...
                    `front_url`,
                    `fs_url`,
                     corp_url, s.corp_id,'',concat(fs_url,'/sys/printLogo.png'),
                     '','',icp,aesKey
               from `sys_info` s
               where id=2 
            """ 
...
def mValidateUser(request,mode,menu_id):
    ...
    if source == 'wx':
        ...
        ret,usr_id,sTimeStamp = wxcpt.DecryptMsg(AccessToken)   
        if (ret !=0):
            errCode = -1
            s = """
                {
        ...
        sql = """select u.usr_id,u.usr_name,ifnull(su.id,0),su.cname,0,time_to_sec(now())-time_to_sec(u.refresh_time),u.expire_time,u.id,u.source
                        from (select u.usr_id,u.usr_name,u.addr_id,ul.refresh_time,ul.expire_time,ul.id,ul.source  from users_login_gy ul left join users_gy u on u.usr_id=ul.usr_id where u.usr_id='%s'  and token='%s' order by ul.login_time desc limit 1) u
    ...
    if rows[0][8]!='wx' and sec>expire_time:
        ...
        return errCode,ToUnicode(s),d_value
    sql = "update users_login_gy set refresh_time=now() where id=%s"%rows[0][7]
    db.executesql(sql)
    d_value = list(rows[0])
...
class DataBaseParent:
    def __init__(self,host,user,password,database):
        ...
        try:
            ...
            self.cursor=self.db_byerp.cursor()
        except:
            pass
...
class cMatType:
    ...
    def loaddata(self,sKey=''):  
        #sql = "delete from mat_type"
        #db.executesql(sql)
        sql="""select id,cname,0,parent_id,0,type_code,ilevel from mat_type where status=1 order by parent_id asc
            """ 
        lT,iN=byerp.select(sql)
        L = []
        for e in lT:
            #sql = "insert into mat_type (id,cname,parent_id,type_code,ilevel) values (%s,'%s',%s,'%s',%s)"%(e[0],e[1],e[3] or 'NULL',e[5],e[6])
            #db.executesql(sql)
            L1 = list(e)
            for e1 in lT:
                if e1[3] == e[0]: 
...
class cUnits:
    ...
    def loaddata(self,sKey=''):         
        ...
        #db.executesql(sql)
        sql="""select id,cname,0,sort from units where type=1 and status =1 order by sort
            """ 
        lT,iN=byerp.select(sql)
        i = 1
        for e in lT:
            #sql = "insert into units (id,cname,sort) values (%s,'%s',%s)"%(e[0],e[1],i)
            #db.executesql(sql)
            i += 1
        self.Lsys= lT
    def get(self):
