...
from share import get_sel_cols,get_gw_type_data2,get_flow_data,get_first_flow_data,get_table_field,get_roles_list,byerp
...
def get_options_2753(search,page_limit,usr_id,request):
    ...
    L = []
    sql = "select distinct top %s cname,cname from mat where status!=0 and cname like '%%%s%%' order by cname asc"%(page_limit,search)
    print ToGBK(sql)
