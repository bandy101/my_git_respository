...
from share        import m_sCorpID,m_sCorpSecret_gy,m_sAgentId_gy,read_access_token_common,write_access_token_common,host_url,my_urlencode
...
def saveListData(pk,mode,value_dict,d_value,request):
    ...
    elif str(menu_id)=='3000001':  
        savePageFormList3000001(pk,mode,value_dict,d_value,request)
    elif str(menu_id)=='3000002':  
        savePageFormList3000002(pk,mode,value_dict,d_value,request)
    if m_prjname == 'hcpra':
...
def savePageFormList3000001(pk,mode,value_dict,d_value,request):
    print request.POST
    push = request.GET.get('push','')
    push = push.replace('/','')
    if str(push) == '1':
        sql = "update _m1501_cgdd set qrd_time=now(),status=3,status1=1 where id=%s"%(pk)
        db.executesql(sql)
        mWxPushMsg_qrd(pk)
    else:
        sql = "update _m1501_cgdd set qrd_time=now(),status=2 where id=%s"%(pk)
        db.executesql(sql)
    return

def savePageFormList3000002(pk,mode,value_dict,d_value,request):
    print request.POST
    push = request.GET.get('push','')
    push = push.replace('/','')
    if str(push) == '1':
        sql = "update _m3000002_dzd set utime=now(),status=1 where id=%s"%(pk)
        db.executesql(sql)
        mWxPushMsg_dzd(pk)
...
def mWxPushMsg_qrd(pk):   
    ...
    toUser = L[0]
    sUrl='%s/complaint/login/login_qy?fid=mtConfirmDetail&func_id=1000004&seq=%s'%(host_url,pk)
    stitle = """采购确认单通知"""
    surl = my_urlencode(sUrl)
    description = """项目名称：%s\r\n供应商名称：%s\r\n采购订单编号：%s\r\n发起时间：%s"""%(L[1],L[2],L[3],L[4])
...
def getQrdData(pk):
    sql = """select ifnull(u.wxqy_id,u.login_id),op.cname,su.cname,cg.sn,cg.qrd_time from _m1501_cgdd cg
...
def mWxPushMsg_dzd(pk):   
    ...
    toUser = L[0].replace(",","|")
    toUser += '|lishijie'
    sUrl='%s/complaint/login/login_qy?fid=mtCheckDetail&func_id=1000004&seq=%s'%(host_url,pk)
    stitle = """材料款对账"""
    surl = my_urlencode(sUrl)
...
def getDzdData(pk):
    sql = """select dz.recv_usr_loginids,op.cname,su.cname,dz.sn,dz.utime 
                from _m3000002_dzd dz
                left join users u on u.usr_id = dz.cid
