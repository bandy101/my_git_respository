...
exec ('from %s.share        import db,dActiveUser,g_data,data_url,front_url,fs_url,m_corp_name,m_muti_lang,byerp'%prj_name) 
exec ('from %s.share        import byerp,oMatType,oUnits'%prj_name) 
...
def getPageForm(request):
    ...
    if mode in ['add','upd'] or str(has_audit) == '0':  
        sql = "select type,step,step_name from menu_form_steps where menu_id =%s and step='%s'"%(menu_id,step)
        rows,iN = db.select(sql)
        if iN>0 and rows[0][0] == 0:
            L.append(list(rows[0][1:]))
        else:
    ...
    names = 'col_name has_add has_del has_order has_import import_name import_url has_import_from_file file_btn_name has_single_import single_name buttons'.split()
...
def getPageBtn(menu_id,mode,step,has_btn,steps):
    ...
    if menu_id == '10202':  #信息管理
        ...
        return L
    if menu_id == '2000003':  #信息发送
        if mode in ['add','upd']:
            L.append(['temp_save','暂存','button','','','','','','','','','','','temporarySave','blue','common/msg_send/saveMsg?is_send=0'])
            L.append(['send','发送','button','','','','','','','','','','','submit','blue','common/msg_send/saveMsg?is_send=1'])
        return L
    if menu_id == '2000012':  #信息发送
        if mode in ['add','upd']:
            L.append(['temp_save','暂存','button','','','','','','','','','','','temporarySave','blue','common/saveMsgSup?is_send=0'])
            L.append(['send','发送','button','','','','','','','','','','','submit','blue','common/saveMsgSup?is_send=1'])
        return L
        ...
        for e in rows:
            ...
            for i in range(0,iCount):
                ...
                if str(rows1[i][17]) =='16':  #多选弹出框
                    ...
                    if rows1[i][-1] == 2754:
                        temp,row[i] = get_options_data_2754(request,value_dict)
                    else:
                    ...
                    if rows1[i][-1] == 2753:
                        temp,row[i] = get_options_data_2753(value_dict)
                    elif rows1[i][-1] == 2755:
                        temp,row[i] = get_options_data_2755(request,value_dict)
                    elif rows1[i][-1] == 2756:
                        temp,row[i] = get_options_data_2756(request,value_dict)
                    elif rows1[i][-1] == 2757:
                        temp,row[i] = get_options_data_2757(request,value_dict)
                    else:
                    ...
                    if rows1[i][-1] == 2758:
                        temp,row[i] = get_options_data_2758(request,value_dict)
                    else:
    ...
    if str(menu_id)=='901':
        ...
        for e in rows:
            ...
            for i in range(0,iCount):
                ...
                if rows1[i][17]==5:
                    single = False
                #print "i=%s row=%s %s"%(i,ToGBK(str(row[i])),rows1[i][-1])
                if str(rows1[i][17]) in ['5','3','6','18','26','32'] and is_view==0:  
                    if rows1[i][-1] == 2754:
                        row[i] = get_options_data_view_2754(row[i])
                    elif rows1[i][-1] == 2758:
                        row[i] = get_options_data_view_2758(row[i])
                    else:
...
def getPageDataEdit(pk,usr_id,menu_id,gw_type,step,mode,has_list,request,has_atta):
    ...
    if len(rows_table)>0:
        ...
        sql ="""SELECT mfc.col_name,mg.has_add,mg.has_del,mg.has_order,mg.has_import,mg.import_name,mg.import_url
                ,has_import_from_file,file_btn_name,has_single_import,single_name,'',mg.field_id
...
def getFormData(pk,field_id,menu_id,usr_id,request,lang_id):
    ...
    for i in range(0,iN1):
        ...
        if str(e[17]) == '15':  #单选弹出框
            ...
            if e[-1] == 2754:
                e[6],e[10] = get_options_data_2754(request,value_dict)
            else:
                e[6] = getDefaultvalue(menu_id,request,rows1[i][18],rows1[i][6],para1,para2)
                e[6] = getDefaultByRefresh(value_dict,rows1[i][-1],field_id,e[6])
                e[10] = get_options_data(menu_id,usr_id,pk,e[19],e[20],e[21],e[6],para1,para2,single)
            ...
            if e[-1] == 2755:
                e[6],e[10] = get_options_data_2755(request,value_dict)
            elif e[-1] == 2756:
                e[6],e[10] = get_options_data_2756(request,value_dict)
            elif e[-1] == 2757:
                e[6],e[10] = get_options_data_2757(request,value_dict)
            else:
                e[6] = getDefaultvalue(menu_id,request,rows1[i][18],rows1[i][6],para1,para2)
                e[6] = getDefaultByRefresh(value_dict,rows1[i][-1],field_id,e[6])
                e[10] = get_options_data_search(menu_id,usr_id,pk,e[19],e[20],e[21],e[6],para1,para2,single,value_dict,e[-2])
            ...
            if e[-1] == 2758:
                e[6],e[10] = get_options_data_2758(request,value_dict)
            else:
                e[6] = getDefaultvalue(menu_id,request,rows1[i][18],rows1[i][6],para1,para2)
                e[6] = getDefaultByRefresh(value_dict,rows1[i][-1],field_id,e[6])
                e[10] = get_options_data_level(menu_id,usr_id,pk,e[19],e[20],e[21],e[6],para1,para2,single)
            ...
            if e[-1] == 2759:
                e[6] = get_options_data_2759(request)
            else:
                e[6] = getDefaultvalue(menu_id,request,rows1[i][18],rows1[i][6],para1,para2)
                e[6] = getDefaultByRefresh(value_dict,rows1[i][-1],field_id,e[6])
        L1.append(e)
        ...
        for e in rows:
            if e[4]==1:
                ...
                if value=='' or str(value)=='NULL':
                    bNew = 1
        if bNew==1 and status !='deleted':  #新添加行
            sql = """insert into %s ("""%(table_name)  #添加数据
            for e in rows:
                if e[4]==0:                       
                    sql += "%s,"%(e[0])
...
def get_mutisel_options(field_id,values):
    ...
    if iN>0:
        ...
        col1 = rows[0][0]
        col2 = rows[1][0]
        sql = "select DISTINCT %s,%s %s and FIND_IN_SET(%s,'%s')"%(col1,col2,from_table,col1,values)
        print sql
        lT,iN = db.select(sql)
        for e in lT:
            L.append([e[0],e[1],1])
...
def get_options_data_2753(value_dict):
    #print request.POST
    cl_name = value_dict.get('cl_name','')
    cl_name = cl_name.replace("'","''") 
    L = []
    sql = "select distinct top 10 cname,cname,1 from mat where status!=0 and cname = '%s' order by cname asc"%(cl_name)
    print ToGBK(sql)
...
def get_options_data_2755(request,value_dict):
    #print request.POST
    cl_name = request.POST.get('cl_name') or value_dict.get('cl_name','')
    cl_name = cl_name.replace("'","''") 
    cl_spec = request.POST.get('cl_spec') or value_dict.get('cl_spec','')
    cl_spec = cl_spec.replace("'","''") 
    L = []
    sql = "select distinct size,size,0 from mat where cname='%s' group by size"%(cl_name)
    #print sql
    rows,iN=byerp.select(sql)
    b = 0
    for e in rows:
...
def get_options_data_2756(request,value_dict):
    #print request.POST
    cl_name = request.POST.get('cl_name') or value_dict.get('cl_name','')
    cl_name = cl_name.replace("'","''") 
    cl_spec = request.POST.get('cl_spec') or value_dict.get('cl_spec','')
    cl_spec = cl_spec.replace("'","''") 
    cl_model = request.POST.get('cl_model') or value_dict.get('cl_model','')
    cl_model = cl_model.replace("'","''") 
    L = []
    sql = "select distinct sepc,sepc,0 from mat where cname='%s' and size='%s' group by sepc"%(cl_name,cl_spec)
    print ToGBK(sql)
    rows,iN=byerp.select(sql)
    b = 0
    for e in rows:
...
def get_options_data_2757(request,value_dict):
    #print request.POST
    cl_name = request.POST.get('cl_name') or value_dict.get('cl_name','')
    cl_name = cl_name.replace("'","''") 
    cl_spec = request.POST.get('cl_spec') or value_dict.get('cl_spec','')
    ...
    cl_model = request.POST.get('cl_model') or value_dict.get('cl_model','')
    cl_model = cl_model.replace("'","''") 
    cl_brand = request.POST.get('cl_brand') or value_dict.get('cl_brand','')
    cl_brand = cl_brand.replace("'","''") 
    L = []
    sql = "select distinct brand,brand,0 from mat where cname='%s' and size='%s' and sepc='%s' group by brand"%(cl_name,cl_spec,cl_model)
    #print sql
    rows,iN=byerp.select(sql)
    b = 0
    for e in rows:
...
def get_options_data_2754(request,value_dict):
    cl_name = request.POST.get('cl_name','')
    cl_name = cl_name.replace("'","''") 
    cl_spec = request.POST.get('cl_spec','')
    cl_spec = cl_spec.replace("'","''") 
    cl_model = request.POST.get('cl_model','')
    cl_model = cl_model.replace("'","''") 
    cl_brand = request.POST.get('cl_brand','')
    cl_brand = cl_brand.replace("'","''") 
    cl_unit = value_dict.get('cl_unit') or ''
    if cl_unit != '':
        unit = int(cl_unit)
    else:
        if cl_spec != '' and cl_model != '' and cl_brand != '' :
            sql = """select isnull(unit,0) from mat m
                      where m.cname='%s' and m.size='%s' and m.sepc='%s' and m.brand='%s' order by m.id desc
                  """%(cl_name,cl_spec,cl_model,cl_brand)
        elif cl_spec != '' and cl_model != '':
            sql = """select isnull(unit,0) from mat m
                      where m.cname='%s' and m.size='%s' and m.sepc='%s' order by m.id desc
                  """%(cl_name,cl_spec,cl_model)
        elif cl_spec != '':
            sql = """select isnull(unit,0) from mat m
                      where m.cname='%s' and m.size='%s' order by m.id desc
                  """%(cl_name,cl_spec)
        else:
            sql = "select isnull(unit,0) from mat where cname='%s' order by id desc"%(cl_name)
        print ToGBK(sql)
        lT,iN=byerp.select(sql)
        if iN>0:
            unit = lT[0][0]
        else:
            sql = "select isnull(unit,0) from mat where cname='%s' order by id desc"%(cl_name)
...
def get_options_data_view_2754(unit):
    unit_name = ''
    for e in oUnits.get():
        if e[0] == unit:
            unit_name = e[1]
...
def get_options_data_2758(request,value_dict):
    #print request.POST
    cl_name = request.POST.get('cl_name','')
    cl_name = cl_name.replace("'","''") 
    cl_spec = request.POST.get('cl_spec','')
    ...
    cl_brand = request.POST.get('cl_brand','')
    cl_brand = cl_brand.replace("'","''") 
    cl_type = value_dict.get('cl_type') or ''
    if cl_type != '':
        type = int(cl_type)
    else:
        if cl_spec != '' and cl_model != '' and cl_brand != '' :
            sql = """select top 1 isnull(mt.id,0) from mat m 
                     left join mat_type mt on m.type = mt.type_code
                      where m.cname='%s' and m.size='%s' and m.sepc='%s' and m.brand='%s' order by m.id desc
                  """%(cl_name,cl_spec,cl_model,cl_brand)
        elif cl_spec != '' and cl_model != '':
            sql = """select top 1 isnull(mt.id,0) from mat m 
            ...
            sql = """select top 1 isnull(mt.id,0) from mat m 
                     left join mat_type mt on m.type = mt.type_code
                     where m.cname='%s' order by m.id desc
                  """%(cl_name)
        print ToGBK(sql)
        lT,iN=byerp.select(sql)
        if iN>0:
...
def get_options_data_view_2758(value):
    label = ''
    for e in oMatType.get():
        if str(e[0]) == str(value):
            label = e[1]
            break
    return label

def get_options_data_2759(request):
    #print request.POST
    cl_name = request.POST.get('cl_name','')
    cl_name = cl_name.replace("'","''") 
    cl_spec = request.POST.get('cl_spec','')
    ...
    cl_brand = request.POST.get('cl_brand','')
    cl_brand = cl_brand.replace("'","''") 
    cl_unit = request.POST.get('cl_unit') or ''
    if cl_unit == '':
        sql = "select isnull(unit,0) from mat where cname='%s' order by id desc"%(cl_name)
        #print ToGBK(sql)
        lT,iN=byerp.select(sql)
    ...
    sql = "select top 1 isnull(code,'') from mat where cname='%s' and size='%s' and sepc='%s' and brand='%s' and unit=%s order by id desc"%(cl_name,cl_spec,cl_model,cl_brand,cl_unit)
    print ToGBK(sql)
    lT,iN=byerp.select(sql)
    if iN>0:
        code = lT[0][0]
