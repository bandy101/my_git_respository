...
def saveDataExt(pk,mode,value_dict,d_value,request):
    ...
    elif saveFunc == 'saveDZSQ': #材料对账申请
        saveDZSQ(pk,mode,value_dict,d_value,request)

    return 

def saveDZSQ(pk,mode,value_dict,d_value,request):
    data = request.POST.get('data','')
    data_list = json.loads(data)
    print data    
    dz_type = data_list.get("dz_type") or 0
    dz_type = int(dz_type)
    sql = """update _m3000004_rkd rk,_m3000002_dzd_list dz
             set rk.dzsq=%s,rk.sq_money=dz.sq_money,rk.dzsq_status=1 where rk.id=dz.rk_id and dz.m_id=%s and dz.is_sel=1"""%(dz_type,pk)
    db.executesql(sql)
    sql = """update _m3000004_rkd rk,_m3000002_dzd_list dz
             set rk.dzsq=%s,rk.sq_money=0,rk.dzsq_status=0 where rk.id=dz.rk_id and dz.m_id=%s and dz.is_sel=0"""%(dz_type-1,pk)
    db.executesql(sql)
    return
