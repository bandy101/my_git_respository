...
def printPdfShd(request):
    ...
    _sql = """select sup_id from users_gy gy 
                left join addr_book bo on bo.id=gy.addr_id  
            where gy.usr_id=%s           
            """%(user_id)
    # print _sql
    _sup ,_sup_ = db.select(_sql)
    _sup = _sup[-1][-1]
    ...
    sql = """select op.cname,sh.sn,sh.ctime,su.cname,su.tel,sh.cusrname,ifnull(sh.qr_code,''),''
             ...
             """%(pk)
    # print sql
    _sql = """select bo.sup_id from _m3000004_shd sh
							left join users_gy gy on gy.usr_id=sh.cid
							left join addr_book bo on bo.id = gy.addr_id
                where sh.id=%s
            """%(pk)
    # print _sql
    _Temps ,_Tlenth = db.select(_sql)
    print _Temps,"sup：",_sup
    rows,iN = db.select(sql)
    if iN == 0 or _sup not in  _Temps[-1]:
        s = """
        {
            本张送货单不存在或您没有查看权限！
        }        """
