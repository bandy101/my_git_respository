...
#微信服务号变量
AppId = 'wx88c1d3c813d448dc'   #APPID
Token = 'HIvEmG8dVTIIJR6220T'
EncodingAESKey = 'o4pHidRQdihjq7W8JhOmFBSQAFmLbpq1ouJZpcsoHxB'
AppSecret = 'a33a6924ec99e7129efd7a69cf7611f5'
...
def getWxUsers():
    ...
    for e in openids:
        ...
        print nickname
        print '--------'
        sql="select usr_id from users_tj where openid='%s'"%openid
        rows,iN = db.select(sql)
        if iN>0:
            sql ="""UPDATE users_tj SET 
                nickname='%s',
                sex = '%s',
                city = '%s',
                province = '%s',
            ...
            sql="""INSERT INTO users_tj(ctime,status,openid,nickname,sex,city,province,country,headimgurl,subscribe_time)
...
def write_access_token(access_token):
    ...
    log_file=os.path.join('/home/webroot/oWorld/complaint/access_token_tj')
