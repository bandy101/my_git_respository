...
TIME_OUT = 30 
try:
    db=DataBaseParent()     #��ϵ���ݿ�
except:
    raise Exception('db connect error')

def ToUnicode(s):#��python���ַ�������unicode���������ַ�����ǰ�����һ��ǰ�u,��ǰ꡶Ա���û�������ã��һ������˳���Ը��ַ������޸ģ�����������ȥ����ǰ�
    try:#����ֱ��ת�����ַ����ģ�ֱ��ת����������ֱ��ת���ģ��Ȳ�֣������
...
def get_mtc_t_data(sDF,type,title=ToUnicode('--��ѡ��--'),single=True):
...
def get_mtc_sys_data(sDF,type,title=ToUnicode('--��ѡ��--'),single=True):
...
def get_dept_data(sDF,title=ToUnicode('--ѡ����--'),single=True):
...
def get_roles_list(sDF,deptid,title=ToUnicode('--ѡ���ɫ--'),single=True):
    if sDF=='':b='1'
    else: b=''
    if title!='':
        L=[['',title,'',b,'']]
...
#�������һ������
def get_gw_type_data1(sDF,title=ToUnicode('--��ѡ��--'),single=True):
...
#������̶�������
def get_gw_type_data2(sDF,parent_id,title=ToUnicode('--�������̶�������--'),single=True):
    lT=[]
    if parent_id!='':
...
def get_first_flow_data(sDF,gw_type,title=ToUnicode('--��ʼ����--'),single=True):
    lT=[]
    if gw_type !='':
...
def get_flow_data(sDF,gw_type,has_flow,title=ToUnicode('--��ѡ��--'),single=True):
    lT=[]
    if gw_type !='':
...
def get_all_tables(sDF,title='',single=True):
    ...
    for e in lT:
        ...
        table_comment = e[1] 
        table_comment = table_comment.replace(ToUnicode('�Զ�������'),'')
        txt = table_name
        if table_comment != '':
            txt += "(%s)"%(table_comment)
...
def get_form_tables(sDF,step_id,title='--ѡ�������--',single=True):
    lT=[]
    if step_id!='':
...
def get_table_field(sDF,table_name,title=ToUnicode('--�ֶ�����--'),single=True):
    lT=[]
...
def get_capital_info(menu_id,usr_id,sDF,para1,para2,title=ToUnicode('--��ѡ��--'),view = False):
    sDF=str(sDF)
    ...
    if usr_id=='':#�ʽ��ʺ��Ǹ�����Ȩ�û���ȡ�ģ���ˣ���usr_idΪ��ʱ��Ӧ�÷��ؿյ��б�
        return L
    ...
    if view==True:
        ...
        if type_name in [ToUnicode('�ֽ�')]:#typeΪ�ǿգ�ȡ��ָ�����͵��ʽ��ʺţ���:0���ֽ��ˣ�1��������
            sql+="AND type=0"
        elif type_name in [ToUnicode('֧Ʊ'),ToUnicode('ת��')]:#typeΪ�ǿգ�ȡ��ָ�����͵��ʽ��ʺţ���:0���ֽ��ˣ�1��������
            sql+="AND type=1"
        else:
            sql+="AND type=2"
    lT,iN=db.select(sql)
...
def get_sup_info(menu_id,usr_id,sDF,para1,para2,title=ToUnicode('--��ѡ��--'),view = False):
    sDF=str(sDF)
...
def get_users_info(menu_id,usr_id,sDF,para1,para2,title=ToUnicode('--��ѡ��--'),view = False):
    sDF=str(sDF)
...
def get_mat_info(menu_id,usr_id,sDF,para1,para2,title=ToUnicode('--��ѡ��--'),view = False):
    sDF=str(sDF)
...
def get_proj_info(menu_id,usr_id,sDF,para1,para2,title=ToUnicode('--��ѡ��--'),view = False):
    sDF=str(sDF)
...
def get_proj_info_by_user(menu_id,usr_id,sDF,type,title=ToUnicode('--��ѡ��--')):
    sDF=str(sDF)
...
def get_ht_info(menu_id,usr_id,sDF,para1,para2,title=ToUnicode('--��ѡ��--'),view = False):
    sDF=str(sDF)
...
def get_cw_data(menu_id,sDF,para1,para2,title='--��ѡ��--',view = False):
    if view == True:
        sql_str = """
          SELECT cw.id                          
...
def get_sql_data_level(usr_id,sDF,txt,para1,para2,title='--��ѡ��--',single=True):
    ldf=[]
    sDF=str(sDF)
    if not sDF is None and sDF != '' and single==False:
        ldf=sDF.split(',')
...
def get_sql_data(usr_id,sDF,txt,para1,para2,title='--��ѡ��--',single=True): 
    ldf=[]
    sDF=str(sDF)
    if not sDF is None and sDF != '' and single==False:
        ldf=sDF.split(',')
...
def get_mtc_t_data1(sDF,type,title='--��ѡ��--',single=True):
    ldf=[]
    sDF=str(sDF)
    if not sDF is None and sDF != '' and single==False:
        ldf=sDF.split(',')
...
def get_dept_data1(sDF,title='--��ѡ��--',single=True):
...
def get_menu_list(sDF,title='--��ѡ��--',single=True):
