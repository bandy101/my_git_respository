...
from share        import AppId_gy,AppSecret_gy,read_access_token_common,write_access_token_common
...
def saveListData(pk,mode,value_dict,d_value,request):
    ...
    elif str(menu_id)=='1501': #采购订单管理
        savePageFormList1501(pk,mode,value_dict,d_value,request)
...
def savePageFormList1501(pk,mode,value_dict,d_value,request):
    ...
    if str(push) == '1':
        ...
        if iN > 0:
            ...
            users = rows1[0][0]
            mWxPushMsg_cgdd(pk,users,sn,proj_name)
        sql = "update _m1501_cgdd set utime=now(),status=1 where id=%s"%(pk)
        db.executesql(sql)
    return
