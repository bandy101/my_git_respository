...
def insert_page(pk,page_name,cname,form_table,form_table_ab,v_order,has_add,sort,has_audit):
    ...
    if iN==0:
        ...
        if str(has_audit) == '1': 
            if page_name == 'all':
                where_sql = " where (D.can_view or gr.role_id in (select role_id from usr_role where usr_id=$s))"
                table_sql = """SELECT DISTINCT D.id,D.title,D.cur_flow_id,D.finish,D.cur_flow_name,D.cur_user_name,D.status_txt,D.type_id,
                    gt.is_pub or FIND_IN_SET($s,D.cybl) or FIND_IN_SET($s,gt.right_users ) or FIND_IN_SET($s,gt.right_depts ) as can_view
                    FROM gw_doc D
                    LEFT JOIN gw_type gt on gt.id = D.type_id
                    WHERE D.type_id=$s  and (gt.is_pub or FIND_IN_SET($s,D.cybl) or FIND_IN_SET($s,gt.right_users ) or FIND_IN_SET($s,gt.right_depts ) or length(gt.right_roles))"""
