...
import httplib
from share import db,dActiveUser,mValidateUser,HttpResponseCORS,ToGBK,ToUnicode,HttpResponseJsonCORS,data_url,fs_url,m_prjname,front_url
...
editor_path = "/home/webroot/data/%s/open/editor_files/"%(m_prjname)
def editor_attach_save(request):
    ...
    ext_arr = {
            ...
            'file' :('doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'htm', 'html', 'txt', 'zip', 'rar', 'gz', 'bz2','7z','pdf','wps'),
            }
    today = datetime.date.today()
    year = today.year
    month = today.month
