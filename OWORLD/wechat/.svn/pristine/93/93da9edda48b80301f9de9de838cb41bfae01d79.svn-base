...
def saveDataExt(pk,mode,value_dict,d_value,request):
    ...
    elif saveFunc == 'addNewMat505':
        addNewMat505(pk,mode,value_dict,d_value,request)
    elif saveFunc == 'saveEmpl': #员工档案
        saveEmpl(pk,mode,value_dict,d_value,request)
    elif saveFunc == 'saveJfqzFh': #甲方签证复函登记
        saveJfqzFh(pk,mode,value_dict,d_value,request)
    elif saveFunc == 'saveProjJsfk': #项目结算审计反馈
        saveProjJsfk(pk,mode,value_dict,d_value,request)
    elif saveFunc == 'saveZchtZF': #战略采购合同材料作废
        saveZchtZF(pk,mode,value_dict,d_value,request)

    return 

def saveZchtZF(pk,mode,value_dict,d_value,request):
    sql = """update _m505_zlcght_list zc,_m50702_zcht_zf_list zf
             set zc.cancel=zf.cancel where zc.id=zf.zcmx_id and zf.m_id=%s"""%(pk)
    db.executesql(sql)

def saveJfqzFh(pk,mode,value_dict,d_value,request):
    sql = """insert into _m430_jfqz_list (m_id,fuh_date,fuh_pass,fuh_memo,save_user,save_addr,fhdj_user,fhdj_date) 
             select %s,fuh_date,fuh_pass,fuh_memo,save_user,save_addr,fhdj_user,fhdj_date from _m430_changelist where id=%s"""%(pk,pk)
    db.executesql(sql)

def saveProjJsfk(pk,mode,value_dict,d_value,request):
    sql = """insert into _m451_xmjs_fk (m_id, fk_usr_name, fk_date, has_new_price, new_price, is_finish, fk_memo, is_new_jf, js_lxr, js_lxfs, sj_lxr, sj_lxfs) 
             select %s, fk_usr_name, fk_date, has_new_price, new_price, is_finish, fk_memo, is_new_jf, js_lxr, js_lxfs, sj_lxr, sj_lxfs from _m451_xmjssj where id=%s"""%(pk,pk)
    db.executesql(sql)

def saveEmpl(pk,mode,value_dict,d_value,request):
    step = request.POST.get('step','')
    if step == '2':
        sql = """update empl e,users u
             set u.dept_id = e.Dept_id
             where e.usr_id = u.usr_id and e.id = %s """%(pk)
    else:
        sql = """update empl e,addr_book ab 
             set ab.license = e.Id_num,ab.name=e.cname,ab.sex=e.sex,ab.birthday=e.Birth_day
             ,ab.bank = e.bank,ab.bank_name = e.bank_uname,ab.bank_no = e.bank_account
             where e.id = ab.emp_id and e.id = %s """%(pk)
    db.executesql(sql)
    return
...
def addNewMat505(pk,mode,value_dict,d_value,request):
    data = request.POST.get('data','')
    data_list = json.loads(data)
    lists = data_list.get('list','')
    cid = d_value[0]
    ...
    for e in lists:
        ...
        if status in ['deleted','exist']: 
            ...
            if iN == 0:
                ...
                cl_mat_code = rows[0][1]
                cl_mat_type = rows[0][2]
            sql = """update _m505_zlcght_list set cl_mat_id=%s,cl_mat_code='%s',cl_mat_type='%s'
                  where gw_id=%s and cl_mat_name='%s' and cl_unit='%s' and cl_size='%s' and cl_brand='%s' and cl_type='%s'
            """%(cl_mat_id,cl_mat_code,cl_mat_type,pk,cl_mat_name,cl_unit,cl_size,cl_brand,cl_type)
            db.executesql(sql)
    return
...
def getAutoCode(cllb):
    ...
    if cllb == '' or str(cllb) == '-1':
        ...
        if iN>0:
            code = lT[0][0]
            code += "-" 
            sql = "select max(cast(replace(ifnull(number,0),'%s','') as SIGNED)) from _m504_CLGL  where number like '%s%%'"%(code,code)
            print ToGBK(sql)
...
def saveVerifyExt(pk,mode,d_value,request):
    ...
    elif saveFunc == 'saveVerifyGQ':  #材料挂起
        saveVerifyGQ(pk,mode,d_value,request)
    elif saveFunc == 'saveVerifyDB':  #材料调拨
        saveVerifyDB(pk,mode,d_value,request)
...
def saveVerifyGQ(pk,mode,d_value,request):
    #print request.POST
    usr_id = d_value[0]
    usr_name = d_value[1]
    data =  request.POST.get('data','')
    data_list = json.loads(data)
    sql = "update _m41108_cailgq_list mx,_m41108_cailgq pd set mx.proj_id=pd.proj_id where pd.id=mx.m_id and pd.id = %s"""%pk
    db.executesql(sql)

    sql = """select mx.id,mx.mat_id,mx.lc_number,mx.vwap,mx.money,pd.proj_id 
            from _m41108_cailgq_list mx
            left join _m41108_cailgq pd on pd.id = mx.m_id
            where pd.id = %s"""%pk
    lT,iN = db.select(sql)
    for e in lT:
        mx_id = e[0]
        ...
        amount = e[4]
        proj_id = e[5]
        sql = "select qty from mat_stock_log where type=6 and mx_id = %s order by id desc"%(mx_id)
        lT1,iN1 = db.select(sql)
        if iN1 == 0: #新增
            sql = """insert into `mat_stock_log` (`type`,`type_name`,`direction`,`gw_id`,`mx_id`,`proj_id`,`mat_id`,`qty`,`in_qty`,`out_qty`,`price`,`amount`,`cid`,`cusrname`,`ctime`)
                         values (6,'挂起',-1,%s,%s,%s,%s,%s,0,%s,%s,%s,%s,'%s',now())
                    """%(pk,mx_id,proj_id,mat_id,qty,qty,price,amount,usr_id,usr_name)
            db.executesql(sql)
            sql = "update mat_stock set qty = qty - %s,amount = amount - %s where mat_id = %s and proj_id= %s"%(qty,amount,mat_id,proj_id)
            db.executesql(sql)
            sql = "select id from mat_stock where old_proj_id=%s and mat_id=%s"%(proj_id,mat_id)     
            lT2,iN2 = db.select(sql)
            if iN2 == 0: #新增
                sql = """INSERT INTO mat_stock (mat_id, qty, price, amount, old_proj_id) 
                         VALUES (%s,%s,%s,%s,%s);"""%(mat_id,qty,price,amount,proj_id)
            else:
                sql = """update mat_stock set qty=qty + %s,amount=amount + %s
                         where id=%s;"""%(qty,amount,lT2[0][0])
            print sql
            db.executesql(sql)
    return

def saveVerifyDB(pk,mode,d_value,request):
    #print request.POST
    usr_id = d_value[0]
    usr_name = d_value[1]
    data =  request.POST.get('data','')
    ...
    sql = """select mx.id,mx.mat_id,mx.number,mx.price,mx.money,pd.dc_proj_id,pd.proj_id
            from _m457_allot_list mx
            left join _m457_allot pd on pd.gw_id = mx.gw_id
            where pd.gw_id = %s"""%pk
    lT,iN = db.select(sql)
    for e in lT:
        mx_id = e[0]
        ...
        dc_proj_id = e[5]
        proj_id = e[5]
        sql = "select qty from mat_stock_log where type=7 and mx_id = %s order by id desc"%(mx_id)
        lT1,iN1 = db.select(sql)
        if iN1 == 0: #新增
            sql = """insert into `mat_stock_log` (`type`,`type_name`,`direction`,`gw_id`,`mx_id`,`proj_id`,`mat_id`,`qty`,`in_qty`,`out_qty`,`price`,`amount`,`cid`,`cusrname`,`ctime`)
                         values (7,'调拨',1,%s,%s,%s,%s,%s,%s,0,%s,%s,%s,'%s',now())
                    """%(pk,mx_id,proj_id,mat_id,qty,qty,price,amount,usr_id,usr_name)
            db.executesql(sql)
            sql = """update mat_stock set qty=qty - %s,amount=amount - %s
                         where mat_id = %s and old_proj_id= %s;"""%(qty,amount,mat_id,dc_proj_id)
            db.executesql(sql)
            sql = "select id from mat_stock where proj_id=%s and mat_id=%s"%(proj_id,mat_id)     
            lT2,iN2 = db.select(sql)
            if iN2 == 0: #新增
                sql = """INSERT INTO mat_stock (mat_id, qty, price, amount, proj_id) 
                         VALUES (%s,%s,%s,%s,%s);"""%(mat_id,qty,price,amount,proj_id)
            else:
                sql = "update mat_stock set qty = qty + %s,amount = amount + %s where id= %s"%(qty,amount,lT2[0][0])
            print sql
            db.executesql(sql)
    return
