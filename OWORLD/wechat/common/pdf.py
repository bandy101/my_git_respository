...
from qr_code import printQrCode,printFACode
def printPDF(request):
    ...
    value_dict,count =  setStaticData(pk,menu_id,field_id)
...
def setStaticData(pk,menu_id,field_id):
    #获取主表数据
    value_dict = dict()    
    ...
    count = 1
    #获取明细表数据
    sql = "select id,table_name,table_sql from menu_pdf_source where menu_id = %s and is_list =1 and field_id=%s;"%(menu_id,field_id)
    print sql
    lT,iN=db.select(sql)
    if iN == 0:
