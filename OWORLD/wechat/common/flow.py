...
def saveAudit(request):
    ...
    sql="""SELECT ifnull(cur_flow_usr_id,0),ifnull(cur_flow_id,0),ifnull(cur_flow_status,0),ifnull(status,0),ifnull(add_view_users,0) FROM gw_doc WHERE id = %s
        """%pk
    #print sql
   
    lT,iN=db.select(sql) #得到基本信息
    cur_usr_id,cur_flow_id,cur_flow_status,doc_status,add_view_users = lT[0]
    ...
    #添加可浏览本条公文的人员
    if add_view_users == 0 and str(is_send) =='1':
        addViewUsers(pk)
...
def addViewUsers(pk):
    ...
    users += cybl
    sql = "select ifnull(right_depts,''),ifnull(right_roles,''),ifnull(right_users,'') from gw_type where id=%s"%(type_id)
    lT,iN = db.select(sql)
    right_depts = lT[0][0] 
    right_roles = lT[0][1] 
    right_users = lT[0][2] 
    #按人员
    if right_users != '':
        users = users + ',' + right_users

    #按部门
    for e in right_depts.split(','):  
        if e is None or e == '':
            continue
        sql = """select group_concat(d1.id) from dept d1
                 left join dept d2 on d1.parent_id = d2.id
                 left join dept d3 on d2.parent_id = d3.id
                 where d1.id = %s or d2.id = %s or d3.id = %s"""%(e,e,e)
        lT,iN = db.select(sql)
        true_depts = lT[0][0] 
        sql = "select ifnull(GROUP_CONCAT(usr_id),'') from users where FIND_IN_SET(dept_id,'%s')"%(true_depts)
        lT,iN = db.select(sql)
        dept_users = lT[0][0] 
        if dept_users != '':
            users = users + ',' + dept_users

    #普通角色
    sql = """select ifnull(GROUP_CONCAT(ur.usr_id),'') from roles r
             left join usr_role ur on r.role_id = ur.role_id 
             where FIND_IN_SET(r.role_id,'%s') and r.is_proj = 0"""%(right_roles)
    lT,iN = db.select(sql)
    role_users = lT[0][0] 
    if role_users != '':
    ...
    #项目角色
    sql = """select ifnull(GROUP_CONCAT(pu.usr_id),'') from roles r
             left join proj_user pu on pu.role_id = r.role_id and pu.proj_id = %s 
             where FIND_IN_SET(r.role_id,'%s') and r.is_proj = 1"""%(proj_id,right_roles)
    lT,iN = db.select(sql)
    role_users = lT[0][0] 
    if role_users != '':
        users = users + ',' + role_users
    print users
    #去重
    usr_list = users.split(',')
    usr_list = list(set(usr_list))    
    users = ','.join(usr_list)
    users += ','

    sql = "update gw_doc set cybl='%s',add_view_users=1 where id=%s"%(users,pk)
    print sql 
    db.executesql(sql)
    return
