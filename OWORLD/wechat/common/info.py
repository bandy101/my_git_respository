...
def getInfoFormView(request,pk,usr_id,title,menu):
    ...
    # 查询阅读人数
    sql = """SELECT COUNT(DISTINCT(usr_id)) FROM bumph_bubbl_read_log WHERE bb_id=%s"""%pk
    persons,iN = db.select(sql)
    ...
    s = """
        ...
        """%(title,menu,infoData,commentData,persons[0][0])
    return HttpResponseCORS(request,s)
...
def readLog(request):
    ...
    sql = """
    SELECT title, usr_name, pic, date_format(read_time,'%%Y-%%m-%%d %%T'), remote_ip FROM
    (SELECT a.bb_id, c.title, a.usr_id, b.usr_name, b.pic, a.read_time, a.remote_ip FROM bumph_bubbl_read_log as a
    LEFT JOIN users as b ON a.usr_id=b.usr_id
    LEFT JOIN bumph_bubbl as c ON a.bb_id=c.id) as d
    WHERE bb_id = %s ORDER BY read_time DESC"""%pk

    rows,iN = db.select(sql)
    names = 'title usr_name pic read_time remote_ip'.split()
    ...
    for e in rows:
        ...
        pic = e[2]
        if pic=='':
            L1[2] = "%s/user_pic/default.jpg"%(fs_url)
        else:
            L1[2] = "%s/user_pic/small_%s"%(fs_url,pic)
        L.append(L1)
