...
class Toolbox:
    ...
    def predict(self, img: np.ndarray, box: tuple=None) -> int:
    ...
    def predictImage(self, img_path: str, box: tuple=None, roi: bool = False, scale: float = None):
        """ 识别指定图片
        - img_path str: 图片路径
        ...
        print(box, self.predict(img, box))

    def predictImages(self, dir_path: str, box: tuple=None, roi: bool = False, scale: float = None):
        """ 识别指定图片目录
        - dir_path str: 图片目录
        - box tuple[None]: 车辆位置, 如果为None则原图识别
        - roi bool[False]: 是否手动选择车辆位置
        ...
        for i in os.listdir(dir_path):
            self.predictImage(path.join(dir_path, i), box, roi, scale)
